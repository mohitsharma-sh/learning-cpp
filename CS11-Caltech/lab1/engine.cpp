...
Range find(vector<RegexOperator*> regex, string const& s) {
    Range match_;

    for (int index=0; index < s.length(); ++index) {
        match_ = findAtIndex(regex, s, index);
        if (match_.end != -1) {
            break;
        }
    }
...
bool match(vector<RegexOperator*> regex, string const&s) {
    Range match_ = find(regex, s);
    bool isStringEmpty = s.length() == 0;
    bool isRegexEmtpy = regex.size() == 0;
    
    bool isEmptyMatch = isStringEmpty && isRegexEmtpy;

    return isEmptyMatch || ((match_.end != 0) && (match_.start == 0) && (match_.end == s.length()));
}
